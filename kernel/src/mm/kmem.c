...
{
  kmem_slab_t *slab = (kmem_slab_t *)((uptr)pmm_alloc(pmm_pagecount_to_order(
                                          SLAB_SIZE / ARCH_PAGE_GRAN)) +
                                      HHDM);
  ...
  if (active_slab->freelist == NULL)
  {
    ...
    } else
    {
      ...
      if (list_is_empty(&cache->slabs_partial))
        ...
        cache->per_cpu_active_slab[curr_cpu_id] = LIST_GET_CONTAINER(
            list_pop_head(&cache->slabs_partial), kmem_slab_t, list_elem);
...
void kmem_free(void *obj, uint size)
{
  ...
  if (g_smp_initialized)
    curr_cpu_id =
        (int)((thread_t *)arch_cpu_read_thread_reg())->assigned_core->id;
  ...
  if (slab->assigned_cpu_id == -1) // Non-active slab.
  {
    ...
    *(void **)obj = slab->freelist;
    ...
    if (slab->freelist_len ==
        1) // The slab was previously full and now is partial.
    {
  ...
  } else if (slab->assigned_cpu_id ==
             curr_cpu_id) // Active slab belonging to the current CPU.
  {
    *(void **)obj = slab->freelist;
    slab->freelist = obj;
    slab->freelist_len++;
...
void kmem_debug()
{
  FOREACH(n, g_cache_list)
  {
    ...
    log("%s - %u %u", cache->name, cache->slabs_partial.length,
        cache->slabs_full.length);
  }
}
