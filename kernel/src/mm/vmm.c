...
#define SEG_INTERSECTS(BASE1, LENGTH1, BASE2, LENGTH2)                         \
  ((BASE1) < ((BASE2) + (LENGTH2)) && (BASE2) < ((BASE1) + (LENGTH1)))
...
static void insert_seg(vmm_addr_space_t *addr_space, uptr base, u64 len,
  ...
  *created_seg = (vmm_seg_t){.base = base,
                             .len = len,
                             .type = type,
                             .list_elem = LIST_NODE_INIT,
                             .addr_space = addr_space};
...
uptr vmm_map_anon(vmm_addr_space_t *addr_space, uptr virt, u64 len,
  ...
  for (uptr addr = 0; addr < len; addr += ARCH_PAGE_SIZE_4K)
    arch_ptm_map(&addr_space->ptm_map, virt + addr, (uptr)pmm_alloc(0),
                 ARCH_PAGE_SIZE_4K);
...
uptr vmm_map_direct(vmm_addr_space_t *addr_space, uptr virt, u64 len,
  ...
  for (uptr addr = 0; addr < len; addr += ARCH_PAGE_SIZE_4K)
    arch_ptm_map(&addr_space->ptm_map, virt + addr, phys + addr,
                 ARCH_PAGE_SIZE_4K);
...
void vmm_init() {
  ...
  g_vmm_kernel_addr_space =
      vmm_new_addr_space(ARCH_HIGHER_HALF_START, ARCH_MAX_VIRT_ADDR);
  ...
  vmm_map_direct(g_vmm_kernel_addr_space,
                 request_kernel_addr.response->virtual_base, 2 * GIB, VMM_FULL,
