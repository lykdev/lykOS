...
typedef enum { ELF_CLASS_32 = 1, ELF_CLASS_64 = 2 } elf_class_t;

typedef enum { ELF_DATA_ENC_LE = 1, ELF_DATA_ENC_BE = 2 } elf_data_enc_t;
...
typedef enum { SH_WRITE = 1, SH_ALLOC = 2, SH_EXECINSTR = 4 } sh_flag_t;
...
void elf_load_exec(elf_object_t *elf_obj, vmm_addr_space_t *addr_space) {
  ...
  for (uint i = 0; i < hdr->phnum; i++) {
    ...
    if (ph->type == PH_LOAD) {
      ...
      file->ops->read(file, ph->offset, ph->memsz,
                      (void *)(vmm_virt_to_phys(addr_space, ph->vaddr) + HHDM));
    }
  }
