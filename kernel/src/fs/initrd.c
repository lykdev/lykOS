...
static vfs_node_ops_t g_node_ops =
    (vfs_node_ops_t){.read = read, .lookup = lookup, .list = list};
...
static void process_entry(ustar_hdr_t *hdr)
{
  ...
  node->vfs_node =
      (vfs_node_t){.type = hdr->type == '5' ? VFS_NODE_DIR : VFS_NODE_FILE,
                   .mp_node = node,
                   .ops = &g_node_ops};
...
void initrd_init()
{
  ...
  g_mountpoint.root_node =
      &LIST_GET_CONTAINER(g_entry_list.head, initrd_entry_t, list_elem)
           ->vfs_node;
